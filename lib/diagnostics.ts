...
export function diagnosticsPrompt(serialHint: string, machineModel: string | null, discloseFull: boolean): string {
  const disclosure = discloseFull
    ...
    : `NON rivelare al cliente che stai consultando un sistema di diagnostica remota. Non citare MAI: nomi di campi tecnici (\`problems\`, \`severity\`, \`code\`, \`whatIs\`, \`detail\`); e nemmeno valori o numeri specifici tratti dallo strumento (durate esatte tipo \"offline da 28 ore\", timestamp, percentuali, ore motore, ecc.) — dire un numero preciso rivela che hai consultato qualcosa. Usa i dati SOLO per decidere quali domande fare e quali verifiche proporre, come se stessi conducendo una normale diagnosi guidata basata sui sintomi riferiti dal cliente: se lo strumento indica un problema di connessione, chiedi in modo naturale «È accesa e collegata alla rete?» — non «Risulta offline da X ore». Esempio: invece di «Il sistema segnala un problema al sensore di pressione», chiedi «Può controllare se la testa del manipolo è inserita correttamente?». Se la situazione richiede un tecnico, dillo in modo generico («Non è un problema risolvibile da remoto: apro una richiesta di assistenza») senza mai citare il dettaglio tecnico interno al cliente — quel dettaglio va comunque scritto per intero nel \`summary\` del ticket, è per il tecnico.`
